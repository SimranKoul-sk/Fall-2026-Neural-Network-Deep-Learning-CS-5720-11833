...
This folder contains the solutions for Home Assignment 2, covering recurrent
...
| File | Question | Topic |
| --- | --- | --- |
| `q1_rnn_text_generation.py` | Q1 | Character-level LSTM text generation + temperature scaling |
| `q2_sentiment_rnn_imdb.py` | Q2 | LSTM sentiment classifier on IMDB + confusion matrix |
| `q3_convolution_operations.py` | Q3 | Convolution with different strides and padding |
| `q4_cnn_feature_extraction.py` | Q4 | Sobel edge detection + max/average pooling |
| `q5_cnn_architectures.py` | Q5 | Simplified AlexNet + ResNet with residual blocks |
...
- `q1_char_lstm.keras` — the trained text-generation model, saved so generation
  can be re-run without retraining
